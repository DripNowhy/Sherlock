...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid" or self.loss_type == "sr":
            losses = (
                -F.logsigmoid(weight * self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(- weight * self.beta * logits) * self.label_smoothing
            )
